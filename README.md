...
./bin/flotilla questions             # list pending agent questions (--json, --watch)
./bin/flotilla answer <name> "text"  # answer an agent's question (unblocks it; --id to disambiguate)
...
`questions`/`answer` are the agent question/answer channel. When an agent hits a decision only its operator can make, it runs `flotilla-ask "…"` in the container and blocks; the question surfaces in `flotilla questions` (and shows the agent as `blocked` in `flotilla list`). The operator replies with `flotilla answer <name> "…"`, which writes the answer into the agent's session dir and unblocks it. Both are derived purely from the filesystem, so they work whether or not the daemon is running — the daemon only adds the proactive inbox notification. Pass `--id <id>` to `answer` when several questions are pending.
...
- **Inbox.** Notable events (`agent_done`, `pr_opened`, `pr_updated`, `submit_skipped`, `fetch_done`, `question`, `question_answered`) are appended to `~/.flotilla/inbox.jsonl`; read them with `flotilla inbox` (`--watch` tails).
...
- **Question/answer channel.** The daemon notices an in-container `flotilla-ask` request, records a `question` event, and marks the agent `blocked` until the operator answers (`question_answered` on reply). The answer path itself is daemon-independent — `flotilla answer` writes straight into the session dir.
