...
func BuildRoot(f *fleet.Fleet) *cobra.Command {
	...
	root.AddCommand(spawnCmd(f), listCmd(f), attachCmd(f), stopCmd(f), rmCmd(f), submitCmd(f), fetchCmd(f), logsCmd(f), daemonCmd(f), inboxCmd(f), questionsCmd(f), answerCmd(f), agentsCmd(), doctorCmd())
	return root
}
